...
process_atl_output = function(
  ...
  if (
    ...
    if (large.file == F) {
      ...
      if (
        ...
        if (
          ...
          biomass <- atlantistools::agg_data(
            spatial.biomass,
            groups = c('species', 'time'),
            fun = sum
          )
          bind.save(biomass, 'biomass', out.dir)
          rm('biomass')
        }
        ...
        if (plot.biomass.timeseries | process.all | plot.all) {
          ...
          bind.save(biomass.age, 'biomass_age', out.dir)
          rm('biomass.age')
          bind.save(biomass.age.invert, 'biomass_age_invert', out.dir)
          rm('biomass.age.invert')
        }
        ...
        if (
          ...
          if (plot.spatial.biomass.seasonal | process.all | plot.all) {
            ...
            bind.save(biomass.box.invert, 'biomass_box_invert', out.dir)
            rm('biomass.box.invert')
          }
        }
        ...
        if (plot.spatial.biomass | process.all | plot.all) {
          ...
          bind.save(spatial.biomass.stanza, 'biomass_spatial_stanza', out.dir)
          rm('spatial.biomass.stanza')
        }
      }
      ...
      if (plot.weight | plot.all | process.all) {
        ...
        bind.save(weight, 'max_weight', out.dir)
        rm('weight')
        bind.save(spatialNumbers, 'spatial_numbers', out.dir)
        rm('spatialNumbers')
        rm('spatialBiomass2')
      }
      ...
      if (
        ...
        bind.save(RN.age, 'RN_age', out.dir)
        rm('RN.age')
        bind.save(SN.age, 'SN_age', out.dir)
        rm('SN.age')
        ...
        if (plot.numbers.timeseries | plot.cohort | plot.all | process.all) {
          ...
          bind.save(numbers, 'numbers', out.dir)
          rm('numbers')
          bind.save(numbers.age, 'numbers_age', out.dir)
          rm('numbers.age')
          bind.save(numbers.box, 'numbers_box', out.dir)
          rm('numbers.box')
        }
        ...
        if (plot.sn.rn | process.all | plot.all) {
          ...
          bind.save(RN.box, 'RN_box', out.dir)
          rm('RN.box')
          bind.save(SN.box, 'SN_box', out.dir)
          rm('SN.box')
        }
      }
    }
    ...
    if (large.file == T) {
      for (i in 1:nrow(group.types)) {
        if (
          load_fgs(param.ls$groups.file)$IsTurnedOn[which(
            load_fgs(param.ls$groups.file)$Name == group.types$species[i]
          )] ==
            0
        ) {
          next()
  ...
  if (plot.catch | plot.spatial.catch | process.all | plot.all) {
    ...
    catchmt <- atlantisom::load_catch(
      dir = atl.dir,
      file_catch = paste0(run.prefix, "Catch.txt"),
      fgs = groups.data
    ) %>%
  ...
  if (plot.mortality | process.all | plot.all) {
    mortality <- atlantistools::load_mort(
      param.ls$mort,
      prm_run = param.ls$run.prm,
      fgs = param.ls$groups.file,
      convert_names = T
    ...
    specificMortality <- atlantistools::load_spec_mort(
      param.ls$specificmort,
      prm_run = param.ls$run.prm,
      fgs = param.ls$groups.file,
      convert_names = T,
      removeZeros = F
