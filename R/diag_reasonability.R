...
diag_reasonability <- function(
  ...
  if (useVariance == T) {
    if (!(any(realBiomass %>% dplyr::distinct(variable) == "var"))) {
      stop(
        " To use variance estimates in reasonability bounds the values of
  ...
  for (acode in speciesCodes) {
    ...
    if (nrow(rb) == 0) {
      ...
      mb <- modelBiomass %>%
        ...
        dplyr::filter(.data$year >= filterTime) %>%
        dplyr::mutate(
          reasonable = (.data$modelBiomass <
            .data$initialBiomass * initBioBounds[2]) &
            (.data$modelBiomass >= .data$initialBiomass * initBioBounds[1])
        ) %>%
        dplyr::mutate(
          modelSkill = calc_mef(.data$modelBiomass, .data$initialBiomass)$mef
        ) %>%
        ...
        dplyr::mutate(
          propAboveUpper = .data$maxBiomass /
            (.data$initialBiomass * initBioBounds[2]) -
            1
        ) %>%
        dplyr::mutate(
          propBelowLower = -.data$minBiomass /
            (.data$initialBiomass * initBioBounds[1]) +
            1
        ) %>%
        dplyr::group_by(.data$code) %>%
        dplyr::mutate(
          maxExceedance = max(.data$propBelowLower, .data$propAboveUpper)
        ) %>%
        dplyr::ungroup()

      test <- "initialBio"
    } else {
      # if complete data present

      if (!useVariance) {
        # use surveybounds argument to determine reasonability
        ...
        mb <- modelBiomass %>%
          ...
          dplyr::mutate(
            reasonable = (.data$modelBiomass <
              max(.data$biomass, na.rm = T) * surveyBounds[2]) &
              (.data$modelBiomass >
                min(.data$biomass, na.rm = T) * surveyBounds[1])
          ) %>%
          dplyr::mutate(
            modelSkill = calc_mef(.data$biomass, .data$modelBiomass)$mef
          ) %>%
          dplyr::mutate(minBiomass = min(.data$modelBiomass)) %>%
          dplyr::mutate(maxBiomass = max(.data$modelBiomass)) %>%
          dplyr::mutate(
            propInitBio = .data$maxBiomass / .data$initialBiomass
          ) %>%
          dplyr::mutate(
            propAboveUpper = .data$maxBiomass /
              (max(.data$biomass, na.rm = T) * surveyBounds[2]) -
        ...
        # bounds based on variance of estimate of biomass
