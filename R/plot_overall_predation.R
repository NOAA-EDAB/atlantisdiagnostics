...
plot_overall_predation <- function(
  data,
  bioindex.file,
  catch.file,
  min.fract = 0.1,
  ...
  data.new = data %>%
    ...
    # dplyr::group_by(Time,date,Prey,Predator) %>%
    dplyr::mutate(
      consumed.pct = consumed.prey / consumed.tot,
  ...
  data.final = data.new %>%
    ...
    tidyr::complete(
      Predator,
      tidyr::nesting(Time, date, Prey),
      fill = list(consumed.pct = 0, consumed.prey = 0, consumed.tot = 0)
    )
  ...
  for (i in 1:length(plot.spp)) {
    ...
    f1 = ggplot2::ggplot(
      ...
      ggplot2::scale_fill_manual(
        values = plot.cols[1:length(unique(data.spp$Predator))]
      ) +
      ggplot2::xlab('date') +
    ...
    grid::grid.newpage()
    grid::grid.draw(gtable:::rbind.gtable(
      ggplot2::ggplotGrob(f3),
      ggplot2::ggplotGrob(f2),
      ggplot2::ggplotGrob(f1),
      size = 'last'
    ))
  }
  dev.off()
