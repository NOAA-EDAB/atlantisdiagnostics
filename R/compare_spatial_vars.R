...
compare_spatial_vars = function(
  ...
  if (ref.data$statistic[1] != data.type) {
    warning(
      'Reference data "statistics" must be the same as data.type argument'
    )
    stop()
  }
  ...
  if (plot == T) {
    ...
    if (tolower(ref.data$var.name[1]) %in% c('catch', 'biomass', 'numbers')) {
      pdf(
        paste0(out.dir, out.name, '.pdf'),
        width = 6 + (3 * length(run.names)),
        height = 10
      )
      for (s in 1:length(spp.names)) {
        ...
        plot.layout = matrix(
          c(
            1,
            1,
            rep(3, length(run.names) + 1),
            2,
            2,
            rep(4, length(run.names) + 1)
          ),
          byrow = T,
          nrow = 2
        )
