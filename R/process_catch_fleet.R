...
process_catch_fleet = function(fishery.prm, catch, groups.file) {
  ...
  for (s in 1:nrow(fgs)) {
    ...
    if (length(spp.vars) == 0) {
      next()
    }

    spp.catch.vars = grep('Catch', spp.vars, value = T)

    if (length(spp.catch.vars) == 0) {
      next()
    }
    ...
    for (f in 1:length(spp.catch.vars)) {
      ...
      fleet.out.ls[[f]] = dplyr::bind_cols(
        data.frame(polygon = (1:nrow(data.fleet)) - 1),
        data.fleet
      ) %>%
        tidyr::gather('time', 'atoutput', -polygon) %>%
        dplyr::mutate(fleet = fleet.names[f], species = fgs$LongName[s]) %>%
