...
diag_maxsize <- function(
  ...
  boxlayer <- jj %>%
    dplyr::group_by(.data$species, .data$polygon, .data$agecl, .data$time) %>%
  ...
  lenWeightParams <- atlantistools::prm_to_df(
    prm_biol,
    fgs,
    group = groups.age.acronym,
    parameter = c("li_a", "li_b")
  )
  ...
  largeFish <- boxlayer %>%
    ...
    dplyr::mutate(
      passW = ifelse(.data$maxMeanWeight < .data$maxObsWeight, TRUE, FALSE)
    ) %>%
    ...
    dplyr::mutate(
      maxLength = (.data$maxMeanWeight / .data$li_a)^(1 / .data$li_b)
    ) %>%
    dplyr::select(-.data$li_a, -.data$li_b) %>%
    dplyr::relocate(.data$maxLength, .after = .data$time) %>%
    dplyr::mutate(
      passL = ifelse(.data$maxLength < .data$maxObsLength, TRUE, FALSE)
    ) %>%
